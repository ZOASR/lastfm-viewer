...
app.use('*', async (c, next) => {
	...
	const devOrigins = ['http://localhost', 'http://127.0.0.1', 'https://localhost'];
	const isDevRequest = origin && devOrigins.some(dev => origin.startsWith(dev));
	...
	if (isDevRequest || isValidRequest) {
		...
		console.warn('Blocked request:', { origin, referer, userAgent: c.req.header('user-agent') });
...
function validateOrigin(origin: string | undefined, referer: string | undefined): boolean {
	...
	return (
		...
		/^[a-zA-Z0-9][a-zA-Z0-9-]*[a-zA-Z0-9]*\.[a-zA-Z]{2,}$/.test(requestDomain) ||
